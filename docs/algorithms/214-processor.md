...
### [검사 1] 인라인 단일 문자 단축키 의심 탐지
- **Rule 2.1.4 (Single Key Shortcut Suspected):** 인라인 키보드 이벤트 핸들러가 존재하나, 코드 내에 특수키 검사 로직(`ctrlKey`, `altKey`, `metaKey`)이 없는 경우.
  - 결과: **[검토 필요]**
  - 가이드: "해당 요소에 특수키(Ctrl, Alt 등)를 확인하지 않는 키보드 이벤트 핸들러가 있습니다. 단일 문자 단축키로 작동하는 경우 제어 수단(끄기/변경)이 있는지 확인하세요."

### [검사 2] 전역 스크립트 기반 단축키 수동 검토
- **Rule 2.1.4 (Character Key Shortcuts):** `addEventListener`로 등록된 전역 단축키는 정적 분석이 어려우므로, 페이지 전체에 대한 수동 검토 가이드를 제공함.
  - 결과: 항상 **[검토 필요]** 리포트 1건 발행.
  - 가이드: "이 페이지에서 Ctrl, Alt 등 특수키 조합 없이 단일 문자(예: 'J', 'K', '?')만으로 작동하는 단축키가 있는지 확인하세요."
