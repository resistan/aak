...
- **Rule 3.1.1 (Uncommon lang code check):** 형식을 만족하더라도, **자주 사용되는 언어 코드 화이트리스트**에 포함되는가?
  ...
  - 가이드: `lang="[코드]"가 감지되었습니다. 올바른 언어 코드인지 확인하세요.`
...
- **Rule 3.1.1 (Technical Validity Passed - Manual Content Check Required):** 명시된 유효한 언어 코드와 실제 페이지의 주요 텍스트 콘텐츠 언어가 일치하는가?
  ...
  - 가이드 (알려진 언어): `[한국어 명칭]([코드])로 설정되어 있습니다. 실제 페이지 본문의 기본 언어가 맞는지 확인하세요.`
  - 가이드 (기타): `lang="[코드]"가 감지되었습니다. 실제 페이지 본문의 기본 언어가 맞는지 확인하세요.`
  - 비고: ko→한국어, en→영어, ja→일본어, zh→중국어 등 주요 12개 언어는 한국어 명칭을 메시지에 포함
