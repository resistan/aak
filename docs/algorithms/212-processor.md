# ABT-2.1.2-Processor: 초점 이동과 표시 v0.13
...
### [검사 3] 전역 초점 가시성 안내
- **Rule 2.1.2 (Focus Order):** 페이지 전체에 대한 수동 검사 안내
  - 결과: **[검토 필요]** (항상 1건 발행)
  - 가이드: "Tab 키를 눌러 모든 링크와 버튼 이동 시 초점(점선 테두리 등)이 명확하게 표시되는지 육안으로 확인하세요."

> **자동화 한계:** `window.getComputedStyle(el)`은 요소가 현재 포커스를 받고 있지 않으면 `:focus` 상태의 스타일을 반환하지 않습니다. 따라서 인라인 스타일로 `outline: none`을 하드코딩한 극단적인 안티 패턴만 자동 탐지하고, 나머지는 수동 검사로 유도합니다.
